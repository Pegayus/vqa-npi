...
class NPI():
    def __init__(self, core, config, log_path, npi_core_dim=256, npi_core_layers=2, verbose=0, device='/cpu:0'):
        ...
        with tf.device(device):
            ...
            self.default_loss = 5 * self.t_loss + self.p_loss
            self.arg_loss = 0.5 * sum([self.t_loss, self.p_loss]) + sum(self.a_losses)
    ...
    def terminate_net(self):
        ...
        """
        p_terminate = tf.keras.layers.Dense(units=2, kernel_regularizer=tf.keras.regularizers.l2(self.rg))(self.h)
    ...
    def key_net(self):
        ...
        hidden = tf.keras.layers.Dense(self.key_dim, activation=tf.keras.activations.elu,
                                                kernel_regularizer=tf.keras.regularizers.l2(self.rg))(self.h)
    ...
    def argument_net(self):
        ...
        for i in range(self.num_args):
            arg = tf.keras.layers.Dense(units=self.arg_depth, kernel_regularizer=tf.keras.regularizers.l2(self.rg),
