...
handling.
"""
from tasks.vqa.env.config import CONFIG
...
class VQAcore():
    def __init__(self, hidden_dim=100, state_dim=128, batch_size=1, device='/cpu:0'):
        """
        Instantiate a VQA Core object, with the necessary hyperparameters.
        """
        ...
        with tf.device(device):
            ...
            self.env_dim = CONFIG["ENVIRONMENT_ROW"] * CONFIG["ENVIRONMENT_COL"] + \
                           CONFIG["PTR_ROW"] * CONFIG["PTR_COL"]  # 10 * 20  +  6 * 10 = 260
            self.arg_dim = CONFIG["ARGUMENT_NUM"] * CONFIG["ARGUMENT_DEPTH"]      # 3 * 17 = 51
            self.program_dim = CONFIG["PROGRAM_EMBEDDING_SIZE"]                   # 15
